...
class CurveEnvUnified:
    def __init__(self, h=128, w=128, max_steps=200, base_seed=BASE_SEED, stage_id=1, curve_config=None):
        ...
        self.steps = 0 
        self.gt_junctions = []
        self.path_mask = np.zeros((h, w), dtype=np.float32)
    ...
    def obs(self):
        ...
        # 2. Wide-Angle Context (Channel 4)
        ...
        # Instead of hard 0/1:
        is_junc = 0.0
        for jp in self.ep.gt_junctions:
            d = np.linalg.norm(np.array(curr) - jp)
            if d < 8.0: is_junc = max(is_junc, np.exp(-(d**2)/(2*3.0**2)))

        # 4. Soft Gaussian Stop Label
        is_stop = 0.0
        # Check against end of ANY valid path
        for path in self.all_paths:
            d = np.linalg.norm(np.array(curr) - path[-1])
            if d < 10.0: is_stop = max(is_stop, np.exp(-(d**2)/(2*4.0**2)))

        return {
            "actor": actor_obs, 
            "critic_gt": crop32(self.gt_map, int(curr[0]), int(curr[1]))[None],
            "junc_gt": np.array([is_junc], dtype=np.float32),
            "stop_gt": np.array([is_stop], dtype=np.float32)
        }

    def step(self, a_idx):
        self.steps += 1
        
        # 1. Action Handling: STOP
        if a_idx == ACTION_STOP_IDX:
            # Check distance to endpoints of ALL available segments
            d_to_ends = [np.linalg.norm(np.array(self.agent) - p[-1]) for p in self.all_paths]
            min_d_to_end = min(d_to_ends)
            
            is_strict = self.stage_config.get('strict_stop', False)
            if min_d_to_end < 5.0:
                # High reward for stopping at a valid terminal tip
                r_stop = 40.0 if is_strict else 25.0
                return self.obs(), r_stop, True, {"msg": "Correct Stop", "stopped_correctly": True}
            else:
                # Heavy penalty for stopping in the middle of a path
                return self.obs(), -10.0, False, {"msg": "False Stop", "stopped_correctly": False}
        ...
        # 3. Path & Memory Logic
        iy, ix = int(ny), int(nx)
        retracing_penalty = 0.0
        if self.path_mask[iy, ix] > 0.5:
            # Significant penalty to prevent the agent from going in circles
            retracing_penalty = -2.0 
        
        # Update agent and mark mask
        self.agent = (ny, nx)
        self.history_pos.append(self.agent)
        self.path_points.append(self.agent)
        self.path_mask[iy, ix] = 1.0

        # 4. Multi-Target Path Selection
        # Calculate distance to every possible path in the image
        dists_to_paths = [get_distance_to_poly(self.agent, p) for p in self.all_paths]
        L_t = min(dists_to_paths)
        best_path_idx = np.argmin(dists_to_paths)
        self.target_poly = self.all_paths[best_path_idx] # Dynamically follow the nearest segment
        
        # Track progress on this specific segment
        if not hasattr(self, 'prev_path_indices'):
            self.prev_path_indices = [0] * len(self.all_paths)
        
        curr_gt_idx = nearest_gt_index(self.agent, self.target_poly)
        progress_delta = curr_gt_idx - self.prev_path_indices[best_path_idx]
        
        # 5. Reward Calculation
        # A. Distance-to-line reward (Log scale)
        dist_diff = self.L_prev - L_t # Positive if we moved closer
        r = np.sign(dist_diff) * np.log1p(abs(dist_diff))
        r = float(np.clip(r, -2.0, 2.0))

        # B. Precision reward (Gaussian centered on the vessel)
        ...
        # C. Progress reward
        if progress_delta > 0:
            r += 1.5 # Reward moving forward
        else:
            r -= 0.5 # Penalize stagnation, backward movement, or target-jumping

        # D. Alignment reward (Smoothness - Stage 2+)
        if self.stage_config.get('stage_id', 1) >= 2:
            lookahead = min(curr_gt_idx + 5, len(self.target_poly) - 1)
            gt_vec = self.target_poly[lookahead] - self.target_poly[curr_gt_idx]
            act_vec = np.array([dy, dx])
            mag_g, mag_a = np.linalg.norm(gt_vec), np.linalg.norm(act_vec)
            if mag_g > 1e-6 and mag_a > 1e-6:
                cos_sim = np.dot(gt_vec, act_vec) / (mag_g * mag_a)
                r += max(0, cos_sim) * 0.5

        # E. Apply Penalties
        r += retracing_penalty
        if self.prev_action != -1 and self.prev_action != a_idx:
            r -= 0.1 # Jitter penalty
        self.prev_action = a_idx

        # 6. Terminal Conditions & State Updates
        self.L_prev = L_t
        self.prev_path_indices[best_path_idx] = max(self.prev_path_indices[best_path_idx], curr_gt_idx)

        # Distance to end of the specific vessel we are currently following
        dist_to_target_end = np.linalg.norm(np.array(self.agent) - self.target_poly[-1])
        reached_end = (dist_to_target_end < 5.0)
        ...
        if reached_end:
            if not self.stage_config.get('strict_stop', False):
                # Auto-termination if strict stop isn't required (early stages)
                r += 20.0
                done = True
            else:
                # In later stages, reward getting to the end but don't finish until STOP is hit
                r += 5.0 
...
def update_ppo(ppo_opt, model, buf_list, clip=0.2, epochs=4, minibatch=32):
    ...
    stop_gt = torch.tensor(np.concatenate([x['obs']['stop_gt'] for x in buf_list]), device=DEVICE)
    ...
    for _ in range(epochs):
        ...
        for s in range(0, N, minibatch):
            ...
            # Updated Model Call
            logits, val, junction_pred, s_pred, _ = model(obs_a[mb], obs_c[mb], ahist[mb])
            ...
            # Total Loss
            loss_j = F.binary_cross_entropy_with_logits(j_pred, junc_gt.squeeze(), pos_weight=torch.tensor(20.0))
            loss_s = F.binary_cross_entropy_with_logits(s_pred, stop_gt.squeeze(), pos_weight=torch.tensor(40.0))
            
            # Combined Loss
            loss = p_loss + 0.5*v_loss + 2.0*loss_j + 2.0*loss_s - 0.01*entropy
