...
class ManualTreeBuilder:
    def __init__(self, model_junc, image_path):
        ...
        self.raw_img = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8)).apply(img).astype(np.float32) / 255.0
    ...
    def init_agent(self, p1, p2):
        ...
        self.history_pos = [(self.agent[0] - 2*uy, self.agent[1] - 2*ux),
        ...
        self.done, self.initialized = False, True
        self.step_count = 0
        self.update_plot()
    ...
    def step(self):
        ...
        # 1. Prediction
        c0 = window_crop(crop32_inference(self.raw_img, curr[0], curr[1]))
        c1 = window_crop(crop32_inference(self.raw_img, p1[0], p1[1]))
        c2 = window_crop(crop32_inference(self.raw_img, p2[0], p2[1]))
        c3 = crop32_inference(self.path_mask, curr[0], curr[1])
        obs_t = torch.tensor(np.stack([c0, c1, c2, c3])[None], dtype=torch.float32, device=DEVICE)
        h16_t = torch.tensor(fixed_window_history(self.ahist, 16, N_ACTIONS)[None], device=DEVICE)

        with torch.no_grad():
            move_logits, _, j_logit, _, _ = self.model(obs_t, obs_t[:,:1], h16_t)
            self.last_probs = torch.softmax(move_logits, dim=1).cpu().numpy()[0]
            self.last_p_junc = torch.sigmoid(j_logit).item()

        # 2. Junction Logic
        if self.junction_cooldown > 0: self.junction_cooldown -= 1
        if self.last_p_junc > 0.7 and self.junction_cooldown == 0:
            too_close = False
            for b in self.stack:
                d = np.sqrt((self.agent[0]-b['pos'][0])**2 + (self.agent[1]-b['pos'][1])**2)
                if d < self.min_branch_dist: too_close = True; break
            
            if not too_close:
                top_idx = np.argsort(self.last_probs)[-2:]
                self.stack.append({'pos': self.agent, 'ahist': copy.deepcopy(self.ahist), 'act': top_idx[0]})
                self.junction_cooldown = 20 # steps
                print(f"🌿 Junction Saved at step {self.step_count}")
                action = top_idx[1]
            else: action = np.argmax(self.last_probs)
        else: action = np.argmax(self.last_probs)

        # 3. Movement
        dy, dx = ACTIONS_MOVEMENT[action]
        ny, nx = self.agent[0] + dy * STEP_SIZE, self.agent[1] + dx * STEP_SIZE
        ...
        # B. Revisit check (Only after step 15 to prevent self-collision at start)
    ...
    def auto_step(self):
        ...
        if self.done:
            if self.stack:
                ...
                self.init_agent(b['pos'], (b['pos'][0] + dy, b['pos'][1] + dx))
            else:
                print("🏁 Tree Complete.")
    ...
    def update_plot(self):
        ...
        self.ax_main.set_title(f"Junc Prob: {self.last_p_junc:.2f} | Step: {self.step_count}")
        self.fig.canvas.draw_idle()

    def on_click(self, event):
        if event.inaxes != self.ax_main or self.initialized: return
        self.clicks.append((event.ydata, event.xdata))
        self.ax_main.plot(event.xdata, event.ydata, 'ro')
        if len(self.clicks) == 2: self.init_agent(self.clicks[0], self.clicks[1])
    ...
    def on_key(self, event):
        if event.key == " ": 
            ...
            if not self.running: self.running = True; self.timer.start()
        elif event.key == "r":
            print("⌨️ R hit: Resetting.")
            self.timer.stop(); self.running = False; self.initialized = False
            self.clicks = []; self.all_segments = []; self.stack = []
            self.global_visited[:] = 0; self.update_plot()
        elif event.key == "q": plt.close()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--img", default="../Road_pngs/1.png")
    parser.add_argument("--weights", default="model_Stage7_Branching_Full_Realism_FINAL.pth", type=str)
