...
class VESCLPhysics:
    """Mathematical utility to calculate VESCL-style centering rewards."""
    
    @staticmethod
    def get_local_com_reward(crop, sigma=1.2):
        """
        Calculates how centered the agent is relative to the vessel's center of mass.
        Returns a value between 0 and 1.
        """
        h, w = crop.shape
        center = h // 2
        
        # 1. Invert if background is light (standard DSA)
        corners = [crop[0,0], crop[0,-1], crop[-1,0], crop[-1,-1]]
        mass_map = 1.0 - crop if np.median(corners) > 0.5 else crop
        
        # 2. Threshold to remove background noise (VESCL localized filtering)
        mass_map = np.clip(mass_map - 0.1, 0, 1)
        total_mass = np.sum(mass_map)
        
        if total_mass < 0.01: # Off track / No vessel visible
            return 0.0
            
        # 3. Calculate Moments (Center of Mass)
        yy, xx = np.mgrid[:h, :w]
        com_y = np.sum(yy * mass_map) / total_mass
        com_x = np.sum(xx * mass_map) / total_mass
        
        # 4. Distance from the exact center (where the agent is)
        dist_to_com = np.sqrt((com_y - center)**2 + (com_x - center)**2)
        
        # 5. Gaussian Reward (highest when dist is 0)
        return np.exp(-(dist_to_com**2) / (2 * sigma**2))

    @staticmethod
    def get_vesselness_reward(crop):
        """Rewards the agent for being on high-intensity vessel pixels."""
        center = crop.shape[0] // 2
        pixel_val = crop[center, center]
        
        # Determine if vessel is dark or light
        corners = [crop[0,0], crop[0,-1], crop[-1,0], crop[-1,-1]]
        is_dark_vessel = np.median(corners) > 0.5
        
        vesselness = (1.0 - pixel_val) if is_dark_vessel else pixel_val
        return np.clip(vesselness, 0, 1)
...
class CurveMakerFlexible:
    ...
    # --- ADD THIS HELPER METHOD INSIDE CurveMakerFlexible ---
    def _draw_rough_curve(self, img, pts, thickness, intensity, rough_prob=0.0):
        """
        Draws a curve that simulates the jagged/pixelated look of the user's image.
        """
        # If no roughness, use the standard smooth AA drawing
        if rough_prob < 0.1:
            self._draw_aa_curve(img, pts, thickness, intensity)
            return

        # 1. Draw standard curve but slightly thicker to accommodate erosion later
        temp = np.zeros_like(img)
        self._draw_aa_curve(temp, pts, thickness, 1.0)
        
        # 2. Add heavy noise specifically to the line structure to break edges
        noise = self.rng.normal(0, 0.5, temp.shape) * rough_prob
        temp = temp + noise
        
        # 3. Threshold to make it "blocky" / aliased
        temp = (temp > 0.4).astype(np.float32) * intensity
        
        # 4. Composite onto image
        img[:] = np.maximum(img, temp)

    # --- REPLACE THE EXISTING sample_curve METHOD WITH THIS ---
    def sample_curve(self, 
        ...
        # --- NEW: DRAW DISTRACTOR CURVES (Background Web) ---
        # These mimic the cell walls/membranes in your image
        if num_distractors > 0:
            for _ in range(num_distractors):
                # Distractors are usually thinner and fainter
                d_thick = self.rng.integers(1, 3)
                d_int = self.rng.uniform(0.1, 0.4) # Faint background lines
                d_pts = self._generate_bezier_points(curvature_factor=1.5, allow_self_cross=True)
                
                # Draw distractor (using standard AA to keep them looking like membranes)
        ...
        # --- CHANGED: Use _draw_rough_curve instead of _draw_aa_curve ---
        self._draw_rough_curve(img, pts_main, thickness, intensity, roughness_prob)
        # Standard clean mask for the Critic/Ground Truth
        ...
        if branches:
            ...
            for _ in range(num_branches):
                ...
                self._draw_rough_curve(img, pts_branch, b_thick, intensity, roughness_prob)
...
class CurveEnvUnified:
    ...
    def obs(self):
        ...
        # Get raw crops
        raw_ch0 = crop32(self.ep.img, int(curr[0]), int(curr[1]))
        raw_ch1 = crop32(self.ep.img, int(p1[0]), int(p1[1]))
        raw_ch2 = crop32(self.ep.img, int(p2[0]), int(p2[1]))
        
        # --- IMPROVEMENT: Apply Local Contrast Normalization ---
        # This makes the faint 1px line "pop" against the gray background
        ch0 = normalize_crop(raw_ch0)
        ch1 = normalize_crop(raw_ch1)
        ch2 = normalize_crop(raw_ch2)
        # -------------------------------------------------------
    ...
    def step(self, a_idx):
        ...
        if a_idx == ACTION_STOP_IDX:
            if dist_to_end < 5.0:
                return self.obs(), 40.0, True, {"reason": "Goal Reached"}
            return self.obs(), -5.0, False, {"reason": "False Stop"}
        ...
        # A. Ground Truth Reward (The "Map")
        L_t = get_distance_to_poly(self.agent, self.ep.gt_poly)
        best_idx = nearest_gt_index(self.agent, self.ep.gt_poly)
        progress = best_idx - self.prev_idx
        
        # B. VESCL Physics Rewards (The "Sensation")
        # Get the current crop at the new position
        current_obs = self.obs()
        local_crop = current_obs['actor'][0] # Use Channel 0 (current image)
        
        r_com = self.vescl.get_local_com_reward(local_crop)
        r_vessel = self.vescl.get_vesselness_reward(local_crop)

        # 4. HYBRID REWARD CALCULATION
        # Combine the "Map" and the "Sensation"
        reward = 0.0
        
        if progress > 0:
            reward += 1.0 * progress # Reward for moving forward along GT
            reward += 2.0 * r_com    # VESCL: Extra reward for being perfectly centered
            reward += 0.5 * r_vessel # VESCL: Extra reward for being on dark pixels
        else:
            reward -= 0.2 # Penalty for standing still or going backward
        ...
        # Termination logic
        reached_goal = dist_to_end < 5.0
        if reached_goal and not self.stage_config.get('strict_stop', False):
            reward += 20.0
            done = True
        ...
        return current_obs, float(reward), done, {"reached_end": reached_goal}
