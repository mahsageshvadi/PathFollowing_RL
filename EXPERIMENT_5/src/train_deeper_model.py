...
# ---------- ENVIRONMENT ----------
# [Keep imports and CurveEpisode dataclass same as before]
...
class CurveEnvUnified:
    ...
    # [Keep generate_tissue_noise same as before]
    ...
    def reset(self, episode_number=None):
        ...
        # [Keep Parameter Resolution Logic same as before]
        ...
        curvature = resolve('curvature_factor', 1.0)
        noise_prob = resolve('noise', 0.0)
        bg_int = resolve('background_intensity', 0.0)
        min_int = resolve('min_intensity', 0.1)
        max_int = resolve('max_intensity', 1.0)
        gap_prob = resolve('gap', 0.0)
        grid_prob = resolve('grid', 0.0)
        ...
        # Generate Curve with Branches potentially enabled
        ...
        self.gt_polys = [p.astype(np.float32) for p in pts_all]
        for poly in self.gt_polys:
            for pt in poly:
                r, c = int(pt[0]), int(pt[1])
                if 0<=r<self.h and 0<=c<self.w: self.gt_map[r,c] = 1.0
        
        self.ep = CurveEpisode(img=img, mask=mask, gt_poly=self.gt_polys[0]) # gt_poly ref mainly for legacy

        # Start Position
        main_poly = self.gt_polys[0]
        ...
        if use_cold_start:
            ...
            # If branches exist, try to start BEFORE the split to teach decision making
            # But here we stick to standard logic for simplicity
            start_idx = 5 if len(main_poly) > 10 else 0
            curr = main_poly[start_idx]
            p1 = main_poly[max(0, start_idx-1)]
            p2 = main_poly[max(0, start_idx-2)]
        ...
        # --- PATH LOCKING VARIABLES ---
        self.locked_path_idx = None  # None = Explore any; Int = Locked to specific path
        self.prev_indices = [start_idx] * len(self.gt_polys) # Track progress on all paths
        self.L_prev = min([get_distance_to_poly(self.agent, p) for p in self.gt_polys])
    ...
    def step(self, a_idx):
        ...
        if a_idx == ACTION_STOP_IDX:
            near_any_end = False
            # Check ends of all paths if unlocked, or specific path if locked
            check_polys = [self.gt_polys[self.locked_path_idx]] if self.locked_path_idx is not None else self.gt_polys
            
            for poly in check_polys:
                d = np.sqrt((self.agent[0]-poly[-1][0])**2 + (self.agent[1]-poly[-1][1])**2)
                if d < 5.0:
                    near_any_end = True
                    break
        ...
        # 1. Determine active path
        if self.locked_path_idx is None:
            # We are not locked yet. Find closest path.
            best_dist = float('inf')
            best_idx = 0
            for i, poly in enumerate(self.gt_polys):
                d = get_distance_to_poly(self.agent, poly)
                if d < best_dist:
                    best_dist = d
                    best_idx = i
            
            # LOCKING CONDITION:
            # If we are very close to one path (best_dist < 3.0) 
            # AND significantly farther from others (others > 8.0)
            # Then we "Commit" to this path.
            # (Note: This logic simplifies bifurcation handling. At the fork, 
            # distances are similar, so no lock. Once we move down a branch, dists diverge.)
            
            if len(self.gt_polys) > 1 and best_dist < 2.0:
                # Find second closest distance
                dists = sorted([get_distance_to_poly(self.agent, p) for p in self.gt_polys])
                second_closest = dists[1]
                
                # If the second closest path is far enough away (> 8px), we have definitely chosen a branch.
                if second_closest > 8.0:
                    self.locked_path_idx = best_idx
                    # print(f"DEBUG: Locked to Path {best_idx}")

            active_poly_idx = best_idx
            L_t = best_dist
        else:
            # We are locked. We ONLY care about the locked path.
            active_poly_idx = self.locked_path_idx
            L_t = get_distance_to_poly(self.agent, self.gt_polys[active_poly_idx])

        # 2. Calculate Reward based on Active Path
        active_poly = self.gt_polys[active_poly_idx]
        prev_idx_for_path = self.prev_indices[active_poly_idx]
        best_idx_on_path = nearest_gt_index(self.agent, active_poly)
        progress_delta = best_idx_on_path - prev_idx_for_path
        self.prev_indices[active_poly_idx] = max(prev_idx_for_path, best_idx_on_path)

        dist_diff = abs(L_t - self.L_prev)
        
        # Alignment Reward
        if L_t < self.L_prev: r = np.log(EPSILON + dist_diff)
        else: r = -np.log(EPSILON + dist_diff)
        r = float(np.clip(r, -2.0, 2.0))
        
        # Precision Boost
        sigma = 1.0
        precision = np.exp(-(L_t**2) / (2 * sigma**2))
        
        # Progress Reward
        if progress_delta > 0: r += precision * 2.0
        elif progress_delta <= 0: r -= 0.1

        # Cosine Similarity
        if self.stage_config.get('stage_id', 1) >= 2:
            lookahead = min(best_idx_on_path + 4, len(active_poly) - 1)
            gt_vec = active_poly[lookahead] - active_poly[best_idx_on_path]
        ...
        if self.prev_action != -1 and self.prev_action != a_idx: r -= 0.05
        self.prev_action = a_idx
        r -= 0.05

        self.L_prev = L_t # Update previous distance

        # 3. Termination
        done = False
        dist_to_end = np.sqrt((self.agent[0]-active_poly[-1][0])**2 + (self.agent[1]-active_poly[-1][1])**2)
        reached_end = (dist_to_end < 5.0)
        
        # Off track?
        # If locked, deviating from locked path kills the run (prevents jumping)
        # If unlocked, standard check
